...
async function main() {
  ...
  passwordHash = data.adminPasswordHash || (initialPassword ? hash(initialPassword) : '');
  if (!data.adminPasswordHash && passwordHash) { data.adminPasswordHash = passwordHash; await writeData(data); }
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'POST' && url.pathname === '/api/admin/login') { const body = await readBody(req); if (!passwordHash || hash(body.password) !== passwordHash) return sendJson(res, 401, { error: 'Contraseña incorrecta' }); const token = crypto.randomBytes(32).toString('hex'); sessions.set(token, Date.now()); return sendJson(res, 200, { ok: true }, { 'Set-Cookie': `brithida_session=${token}; HttpOnly; SameSite=Lax; Path=/; Max-Age=86400` }); }
